...
from data_gen import load_test_data_for_rnn,load_train_data_for_rnn,load_test_data_for_cnn, load_train_data_for_cnn,erath_data_transform,sea_mask_rnn,sea_mask_cnn,load_train_data_for_kde
from loss import  NaNMSELoss,NaNKDEMSELoss
from model import LSTMModel,CNN,ConvLSTMModel,BILSTMModel,GRUModel
from utils import calculate_kde_weight_renorm,lat_weight
...
#	filter Antatctica
    ...
    if cfg['modelname'] in ['LSTM','BILSTM','GRU','KDE_LSTM','KDE_BILSTM','KDE_GRU']:
        if os.path.exists(cfg['inputs_path']+cfg['product']+'/'+str(cfg['spatial_resolution'])+'/weight.npy'):
            weight =np.load(cfg['inputs_path']+cfg['product']+'/'+str(cfg['spatial_resolution'])+'/weight.npy')
        else:
            print("calculating kde_weight")
            #weight = calculate_kde_weight_renorm(y,cfg,mask,scaler_y)
            weight = lat_weight(y,mask)
            print("calculate kde_weight done")
            np.save(cfg['inputs_path']+cfg['product']+'/'+str(cfg['spatial_resolution'])+'/weight.npy',weight)
    ...
    for num_ in range(cfg['num_repeat']):
        # prepare models
	#Selection model
        if cfg['modelname'] in ['LSTM','KDE_LSTM']:
            lstmmodel_cfg = {}
            lstmmodel_cfg['input_size'] = cfg["input_size"]
            lstmmodel_cfg['hidden_size'] = cfg["hidden_size"]
            lstmmodel_cfg['out_size'] = 1
            model = LSTMModel(cfg,lstmmodel_cfg).to(device)
        elif cfg['modelname'] in ['BILSTM','KDE_BILSTM']:
            lstmmodel_cfg = {}
            lstmmodel_cfg['input_size'] = cfg["input_size"]
            lstmmodel_cfg['hidden_size'] = cfg["hidden_size"]
            lstmmodel_cfg['out_size'] = 1
            model = BILSTMModel(cfg,lstmmodel_cfg).to(device)
        elif cfg['modelname'] in ['GRU','KDE_GRU']:
            lstmmodel_cfg = {}
            lstmmodel_cfg['input_size'] = cfg["input_size"]
            lstmmodel_cfg['hidden_size'] = cfg["hidden_size"]
            lstmmodel_cfg['out_size'] = 1
            model = GRUModel(cfg,lstmmodel_cfg).to(device)   
 ...
 #  train way for LSTM model
                    ...
                    elif cfg["modelname"] in ['KDE_LSTM','KDE_BILSTM','KDE_GRU']:
                        x_batch, y_batch, aux_batch, _, _,weight_batch = load_train_data_for_kde(cfg, x, y, static, scaler_y,weight)
                        x_batch = torch.from_numpy(x_batch).to(device)
                        aux_batch = torch.from_numpy(aux_batch).to(device)
                        y_batch = torch.from_numpy(y_batch).to(device)
                        weight_batch = torch.from_numpy(weight_batch).to(device)
                        aux_batch = aux_batch.unsqueeze(1)
                        aux_batch = aux_batch.repeat(1, x_batch.shape[1],1)
                        x_batch = torch.cat([x_batch, aux_batch], 2)
                        pred = model(x_batch, aux_batch)
 ...
 # ------------------------------------------------------------------------------------------------------------------------------
                    if cfg["modelname"] in ['KDE_LSTM','KDE_BILSTM','KDE_GRU']:
                        loss = NaNKDEMSELoss.fit(cfg, pred.float(), y_batch.float(),weight_batch.float())
                    else:
